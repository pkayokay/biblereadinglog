...
class BackButtonComponent < ViewComponent::Base
  ...
  erb_template <<-ERB
    <div class="mt-6  mb-6 mx-auto max-w-2xl w-full flex items-center justify-between px-4 sm:px-0">
      ...
      <% end %>
      <% if middle_content %>
        <div class="absolute text-center -translate-x-1/2 left-1/2 top-7">
          <%= middle_content %>
        </div>
      <% end %>
      <%= right_content %>
