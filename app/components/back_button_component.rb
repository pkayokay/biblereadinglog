...
class BackButtonComponent < ViewComponent::Base
  ...
  erb_template <<-ERB
    <div class="mt-6 sm:mt-8 mb-5 sm:mb-7 mx-auto max-w-2xl w-full flex items-center justify-between px-4 sm:px-0">
      ...
      <% end %>
      <% if middle_content %>
        <div class="absolute text-center -translate-x-1/2 left-1/2 top-5 sm:top-7">
          <%= middle_content %>
        </div>
      <% end %>
      <%= right_content %>
